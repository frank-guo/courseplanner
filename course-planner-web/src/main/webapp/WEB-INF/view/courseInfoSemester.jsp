...
<html>
    <head>
        ...
        <link href="<c:url value='/static/style-coursinfo.css' />" rel="stylesheet">
        <script type="text/javascript" src="<c:url value='/static/jquery-1.11.0.min.js'/>"></script>
		<script type="text/javascript"  src="<c:url value='/static/course-info.js'/>"></script>
		<script>
    ...
    <span id="guid">Click on CourseID to show the section details For Specified Semester in Text Box</span><br/>
	...
	<% for (String c : list) {%>
	<tr>
	  	<td id=courseid onclick="showCourseDetails('<%=c%>','${semesterId}')"><u><%=c%></u></td>
